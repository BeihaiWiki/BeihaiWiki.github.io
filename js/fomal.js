...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | BeihaiWiki🌊")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'RPDBZ-HTJ3J-H2JFP-DXDXX-YSAP5-SPFFD',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 北海Wiki🌊") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('北海Wiki🌊的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🧡🧡🧡');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
//   var start = new Date("2022/12/08 00:00:00"); // 旅行者1号开始计算的时间
//   var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
//   var unit = (dis / 149600000).toFixed(6);  // 天文单位
//   // 网站诞生时间
//   var grt = new Date("2022/12/08 00:00:00");
...
//   (currentTimeHtml =
//     hnum < 18 && hnum >= 9
//     ? `<img class='boardsign' src='https://img.shields.io/badge/辉仔小屋-科研摸鱼-1ade23?style=social&logo=Docker' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀 <br> 再看看那个光点，它就在这里，这是家园，这是我们 —— 你所爱的每一个人，你认识的一个人，你听说过的每一个人，曾经有过的每一个人，都在它上面度过他们的一生✨ </div>`
//     : `<img class='boardsign' src='https://img.shields.io/badge/F%E5%B0%8F%E5%B1%8B-%E4%B8%8B%E7%8F%AD%E4%BC%91%E6%81%AF%E5%95%A6%F0%9F%8C%99-6adea8?style=social&logo=coffeescript' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀 <br> 再看看那个光点，它就在这里，这是家园，这是我们 —— 你所爱的每一个人，你认识的一个人，你听说过的每一个人，曾经有过的每一个人，都在它上面度过他们的一生✨ </div>`),
...
function createtime() {
  ...
  var e = new Date("2022/12/08 00:00:00"), t = Math.trunc(234e8 + (now - e) / 1e3 * 17), a = (t / 1496e5).toFixed(6), o = new Date("2022/12/08 00:00:00"), r = (now - o) / 1e3 / 60 / 60 / 24, i = Math.floor(r), n = (now - o) / 1e3 / 60 / 60 - 24 * i, s = Math.floor(n); 1 == String(s).length && (s = "0" + s); var l = (now - o) / 1e3 / 60 - 1440 * i - 60 * s, g = Math.floor(l); 1 == String(g).length && (g = "0" + g);
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://beihai.wiki/img/background.webp"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
